...
int main(int argc, char *argv[])
{
    ...
    {
        printf("run failed \n");
        printf("./recoderdemo argv[1] argv[2] argv[3] argv[4] argv[5] argv[6] argv[7]\n");
        ...
        printf(" argv[7]: input YUV format (0:I420; 1:YV12; 2:NV12; 3:NV21) \n");
        return -1;
    }
    ...
    sscanf(argv[6], "%32dx%32d", &demoRecoder.nSrcWidth,&demoRecoder.nSrcHeight);

    sscanf(argv[7], "%32dx%32d", &demoRecoder.ndstWidth,&demoRecoder.ndstHeight);

    //VideoEncodeConfig videoConfig;
    memset(&demoRecoder.videoConfig, 0x00, sizeof(VideoEncodeConfig));

    demoRecoder.videoConfig.nType       = atoi(argv[4]);
    ...
    {
        case 0:
            demoRecoder.videoConfig.nInputFormat = VENC_PIXEL_YUV420P;
            break;
        case 1:
            demoRecoder.videoConfig.nInputFormat = VENC_PIXEL_YVU420P;
            break;
        case 2:
            demoRecoder.videoConfig.nInputFormat = VENC_PIXEL_YUV420SP;
            break;
        case 3:
            demoRecoder.videoConfig.nInputFormat = VENC_PIXEL_YVU420SP;
            break;
        default:
            logw("use the default yuv format:VENC_PIXEL_YUV420P");
            demoRecoder.videoConfig.nInputFormat = VENC_PIXEL_YVU420P;
            break;
    }
