...
/*
**********************************************************************
*                   StreamManagerFlushStreamWithHoldFrames
*
*Description: free the frames whitch had submitted to decoder but hold nHoldFrames frames.
*
*Arguments  : p          Created by StreamManagerCreate function;
*             nHoldFrames the num of frames holding in the streamManager.
*
*Return     : result;
*               = 0;    succeeded;
*               = -1;   failed.
*
*Summary    : After returned, it will hold nHoldFrames frames which had submitted to decoder.
*
**********************************************************************
*/
int StreamManagerFlushStreamWithHoldFrames(StreamManager* p, int nHoldFrames)
{
    int  nFlushPos;
    int i;
    if(p == NULL || nHoldFrames < 0 || nHoldFrames > p->streamFifo.nMaxFrameNum*MIN_PREVERSE_RATIO)
    {
        loge("error.");
        return -1;
    }
    ...
    nFlushPos = (p->streamFifo.nReadPos - nHoldFrames - 1 + p->streamFifo.nMaxFrameNum)%
                p->streamFifo.nMaxFrameNum;
    if(nFlushPos >= p->streamFifo.nWritePos)
    {
        for(i=p->streamFifo.nWritePos; i<=nFlushPos; i++)
        {
            if(p->streamFifo.pFrames[i].pData != NULL)
            {
                free(p->streamFifo.pFrames[i].pData);
                p->streamFifo.pFrames[i].pData = NULL;
            }
        }
    }
    else
    {
        if(p->streamFifo.nWritePos > p->streamFifo.nReadPos)
        {
            for(i=0; i<=nFlushPos; i++)
            {
                if(p->streamFifo.pFrames[i].pData != NULL)
                {
                    free(p->streamFifo.pFrames[i].pData);
            ...
            for(i=p->streamFifo.nWritePos; i<p->streamFifo.nMaxFrameNum; i++)
            {
                if(p->streamFifo.pFrames[i].pData != NULL)
                {
                    free(p->streamFifo.pFrames[i].pData);
