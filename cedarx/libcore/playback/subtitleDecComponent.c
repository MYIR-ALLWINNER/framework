...
int SubtitleDecComptStreamIndexValid(SubtitleDecComp* p, int nStreamIndex)
{
    if(p == NULL)
    {
        logv("no subtitle decode component.");
        return 0;
    } else if ((nStreamIndex < 0) || (nStreamIndex >= p->nStreamCount)) {
        logv("nStreamIndex = %d, p->nStreamCount = %d, audio stream index invalid.",
