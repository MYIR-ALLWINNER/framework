...
Player* PlayerCreate(void)
{
    ...
    p->nPlayRate.mSpeed = XAUDIO_TIMESTRETCH_STRETCH_DEFAULT;
                              ...
                              int             nStreamIndex)
{
    ...
    {
        ret = VideoDecComptStreamIndexValid(p->pVideoDecComp, nStreamIndex);
        return ret;
    }
    else if(eMediaType == MEDIA_TYPE_AUDIO)
    {
        pthread_mutex_lock(&p->audioDecoderMutex);
        ret = AudioDecComptStreamIndexValid(p->pAudioDecComp, nStreamIndex);
    ...
    else
    {
        ...
        ret = SubtitleDecComptStreamIndexValid(p->pSubtitleDecComp, nStreamIndex);
...
void PlayerSetFloatSpeed(Player* pl, float nSpeed)
{
    PlayerContext* p = (PlayerContext*)pl;
    p->pAvTimer->SetSpeed(p->pAvTimer, 1000 * nSpeed);
    VideoRenderSetVideoPlayRate(p->pVideoRender, (int) 1 / nSpeed);
    logd(" set float speed %f.", nSpeed);
    return;
}
