...
//maybe set 1 at Android N, because it maybe wait a long time when system time was changed.
#define REPLACE_SEM_TIMED_WAIT  (0)
...
int AwMessageQueueFlush(AwMessageQueue* mq)
{
    ...
    logv("%s, flush messages.", mqCtx->pName);
    ...
    else
    {
...
#else
        int64_t time_us = time_ms * 1000;
        int64_t timeWait = 100;//min usleep 100us.
        int64_t timeWaitMax = 1000;//max usleep 1000us.
        int64_t elapsedUs = 0;
        int64_t start_time = CdxMonoTimeUs();
        ...
        do {
            ...
            if (timeWait >= timeWaitMax)
                ...
                timeWait <<= 1;
            elapsedUs = CdxMonoTimeUs() - start_time;
        } while(elapsedUs <= time_us);
#endif
    }
