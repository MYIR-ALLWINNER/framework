    ...
    while (payload_size > 0)
    {
        ...
        *q++ = 0x10 | ts_st->cc | (write_pcr ? 0x20 : 0);
        ...
        if (is_start)
        {
            ...
            *q++ = 0x01;
            private_code = 0;
            if (st->codec.codec_id == MUX_CODEC_ID_H264 || st->codec.codec_id == MUX_CODEC_ID_H265)
            {
                *q++ = 0xe0; //for h264 and h265
            }
...
cdx_int32 TsWritePacket(TsMuxContext *s, CdxMuxerPacketT *pkt)
{
    ...
    if(st->codec.codec_id == MUX_CODEC_ID_H264 || st->codec.codec_id == MUX_CODEC_ID_H265)
    {
        ...
        if ((state & 0x1f) != 9 && st->codec.codec_id == MUX_CODEC_ID_H264)
        {
            cdx_uint8 data[6];
        ...
        else if ((state & 0x1f) != 9 && st->codec.codec_id == MUX_CODEC_ID_H265)
        {
            cdx_uint8 data[7];
        ...
        //insert extradata along with IDR frame.
        cdx_uint8 idr_flag = 0;
        if(st->codec.codec_id == MUX_CODEC_ID_H264)
            idr_flag = 0x65;
        else if(st->codec.codec_id == MUX_CODEC_ID_H265)
            idr_flag = 0x26;
        if(st->firstframeflag || *((char*)pkt->buf+4) == idr_flag)
        {
