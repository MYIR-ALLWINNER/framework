...
static const cdx_int32 aac_rate_table[] = { 96000, 88200, 64000, 48000, 44100, 32000, 24000, 22050, 16000, 12000, 11025, 8000, 7350 };

static cdx_int32 aac_sample_rate_index(cdx_int32 sampleRate)
{
    cdx_int32 i = 0;
    for( i = 0; i < 13 ; i++)
    {
        if(aac_rate_table[i] == sampleRate)
            return i;
    }
    return -1;
}
...
int setAudioInfo(MuxMOVContext *mov, MuxerAudioStreamInfoT *p_a_info)
{
    ...
    trk->vos_data[0] = /*0x14*/(2<<3) | (aac_sample_rate_index(p_audio->sample_rate)>>1);
    trk->vos_data[1] = 0x10;
