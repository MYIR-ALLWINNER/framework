...
static cdx_int32 __CdxTcpStreamRead(CdxStreamT *stream, void *buf, cdx_uint32 len)
{
    ...
    if(impl->fpTcpStream)
    {
        fwrite(buf, 1, ret, impl->fpTcpStream);
        sync();
    }
#endif
...
static cdx_int32 __CdxTcpStreamClose(CdxStreamT *stream)
{
    ...
    if(impl->fpTcpStream)
    {
        fclose(impl->fpTcpStream);
        impl->fpTcpStream = NULL;
    }
#endif
    return 0;
}
...
#if SAVE_TCP_STREAM
        impl->fpTcpStream = fopen("/data/camera/fpTcpStream.es", "wb+");
        if (!impl->fpTcpStream)
        {
            CDX_LOGE("open fpTcpStream debug file failure errno(%d)", errno);
        }
#endif
    return &impl->base;
}
