...
#if defined(SAVE_LOG)
    p->fp = fopen("/data/camera/debug.log", "wb");
#endif
...
int CacheManagerRequestCache(HttpCacheManagerT *p, HttpCacheT **ppCache)
{
    ...
    *ppCache = pCache;
    ...
    logv("request cache pCache(%p), p->cacheFifo.nWritePos=%d,"
        ...
        char buf[4096];
        fprintf(p->fp, "\n");
        sprintf(buf, "request cache pCache(%p), p->cacheFifo.nWritePos=%d,"
         " pCache->nCacheSize=%d, pCache->nNodeNum=%d, nNodeSize=%d, s-e(%lld, %lld)",
            pCache, p->cacheFifo.nWritePos-1,
            pCache->nCacheSize, pCache->nNodeNum, p->nNodeSize, pCache->nStartPos, pCache->nEndPos);
        fprintf(p->fp, "%s\n", buf);
    ...
    for(i = 0; i < p->cacheFifo.nCacheNum; i++)
    {
        ...
        if(pos >= c->nStartPos && pos < c->nStartPos + c->nDataSize + c->nPassedSize)
        {
            ...
            if(pNode != NULL)
            {
                ...
                c->nNodeReadPos = pNode->nLength - (offset - pos);
                c->nCurNodeNum = j;
                p->nReqDataPos = pos;
                ...
                logv("CacheManagerSeekTo, pos=%lld seek in cache(%p)[%lld,%lld], cacheFifo.nReadPos=%d, "
                     "cache nDataSize=%d, nPassedSize=%d, "
                     "c.tmpNode(%p),c.pHead(%p), c.nNodeValidSize(%d).",
                    pos, c, c->nStartPos, c->nEndPos, p->cacheFifo.nReadPos,
                    c->nDataSize, c->nPassedSize,
                    c->tmpNode, c->pHead, c->nNodeValidSize);

#if defined(SAVE_LOG)
                char buf[4096];
                fprintf(p->fp, "\n");
                sprintf(buf, "CacheManagerSeekTo, pos=%lld seek in cache(%p)[%lld,%lld], cacheFifo.nReadPos=%d, "
                    "cache nDataSize=%d, nPassedSize=%d, "
                    "c.tmpNode(%p),c.pHead(%p), c.nNodeValidSize(%d).",
                    pos, c, c->nStartPos, c->nEndPos, p->cacheFifo.nReadPos,
                    c->nDataSize, c->nPassedSize,
                ...
                //**  need to update other cache, reset c->pHead, update nDataSize and nPassedSize:
                //*1. caches behind current cache: because c->pHead is NULL after read.
                //*2. the cache was being requested just before seeking back: c->pHead is not the origin head,
                //**  MUST reset it for next RequestData.
                int k;
                for(k = 0; k < p->cacheFifo.nCacheNum; k++)
                {
                    c = &p->cacheFifo.pCaches[k];
                    if(c->pOriHead != NULL && k != p->cacheFifo.nReadPos)
                    {
                        c->pHead = c->pOriHead;
                        if(c->nPassedSize != 0)
                        {
            ...
            {
                logw("pos=%lld, start=%lld, ds=%d, ps=%d", pos, c->nStartPos, c->nDataSize, c->nPassedSize);
                break;
            }
        }
    }
...
int CacheManagerAddData(HttpCacheManagerT *p, HttpCacheT *c, HttpCacheNodeT *node)
{
    ...
    logv("add data, node=%p, size=%d, cache=%p, c->nDataSize=%d, c->nPassedSize=%d,[%lld, %lld],"
        "p->cacheFifo.nReadPos=%d, p->cacheFifo.nWritePos=%d, c=%p, c->pHead=%p, c->pTail=%p, c->nCurNodeNum=%d",
        newNode, node->nLength, p, c->nDataSize, c->nPassedSize, c->nStartPos, c->nEndPos,
        p->cacheFifo.nReadPos, p->cacheFifo.nWritePos, c, c->pHead, c->pTail, c->nCurNodeNum);

#if defined(SAVE_LOG)
        char buf[4096];
        fprintf(p->fp, "\n");
        sprintf(buf, "add data, node=%p, size=%d, cache=%p, c->nDataSize=%d, c->nPassedSize=%d,[%lld, %lld],"
            "p->cacheFifo.nReadPos=%d, p->cacheFifo.nWritePos=%d, c=%p, c->pHead=%p, c->pTail=%p, c->nCurNodeNum=%d",
            newNode, node->nLength, p, c->nDataSize, c->nPassedSize, c->nStartPos, c->nEndPos,
            p->cacheFifo.nReadPos, p->cacheFifo.nWritePos, c, c->pHead, c->pTail, c->nCurNodeNum);
        fprintf(p->fp, "%s\n", buf);
...
int CacheManagerRequestData(HttpCacheManagerT *p, int nRequireSize, void *buf)
{
    ...
    logv("nRequireSize=%d, c->nNodeValidSize=%d, c->cacheFifo.nReadPos,nWritePos=%d,%d, "
        "pCaches=%p, c->tmpNode(%p), curNodeNum=%d, nDatasize=%d, head=%p, pCaches[%d][%lld,%lld]",
        nRequireSize, c->nNodeValidSize, p->cacheFifo.nReadPos, p->cacheFifo.nWritePos,
        c, c->tmpNode, c->nCurNodeNum, c->nDataSize, c->pHead, p->cacheFifo.nReadPos, c->nStartPos, c->nEndPos);

#if defined(SAVE_LOG)
    {
    char buf[4096];
    fprintf(p->fp, "\n");
    sprintf(buf, "nRequireSize=%d, c->nNodeValidSize=%d, c->cacheFifo.nReadPos,nWritePos=%d,%d, "
        "pCaches=%p, c->tmpNode(%p), curNodeNum=%d, nDatasize=%d, head=%p, pCaches[%d][%lld,%lld]",
        nRequireSize, c->nNodeValidSize, p->cacheFifo.nReadPos, p->cacheFifo.nWritePos,
        c, c->tmpNode, c->nCurNodeNum, c->nDataSize, c->pHead, p->cacheFifo.nReadPos, c->nStartPos, c->nEndPos);
    fprintf(p->fp, "%s\n", buf);
    ...
    if(c->nNodeValidSize > 0 && c->tmpNode != NULL)
    {
        ...
        else
        {
            ...
            if(c->nCurNodeNum == c->nNodeNum)
            {
                ...
                char buf[4096];
                fprintf(p->fp, "\n");
                sprintf(buf, "xxx 0 c->nDataSize=%d, c->nPassedSize=%d",
                    c->nDataSize, c->nPassedSize);
                fprintf(p->fp, "%s\n", buf);
                }
#endif
    ...
    while(1)
    {
        ...
        else
        {
            ...
            if(c->nCurNodeNum == c->nNodeNum)
            {
                ...
                char buf[4096];
                fprintf(p->fp, "\n");
                sprintf(buf, "xxx 1 c->nDataSize=%d, c->nPassedSize=%d",
                    c->nDataSize, c->nPassedSize);
                fprintf(p->fp, "%s\n", buf);
                }
#endif
...
int CacheManagerSkipForProbeCache(HttpCacheManagerT *p, cdx_int64 pos)
{
    HttpCacheT *c = NULL;
    HttpCacheNodeT *node = NULL;
    cdx_int64 needSize = pos;
    int readSize = 0;
    int cacheIdx = 0;
    ...
    if(p->probeNode != NULL)
    {
        logd("node=%p, node->nLength=%d, baseOffset=%lld, p->probeNodeIdx=%d, cacheidx=%d",
            p->probeNode, p->probeNode->nLength, pos,
            p->probeNodeIdx, p->probeCacheIdx);
        node = p->probeNode;
        if(node->nLength - p->probeNodePos >= pos)
        {
            p->probeNodePos += pos;
            readSize += needSize;
            pthread_mutex_unlock(&p->mutex);
    ...
    logv("p->probeCacheIdx=%d, p->probeNodeIdx=%d, needSize=%lld, c->pProbeHead=%p",
        p->probeCacheIdx, p->probeNodeIdx, needSize, c->pProbeHead);
    while(1)
    {
        if(needSize <= 0)
            break;
        ...
        logv("needSize=%lld, p->probeNodeIdx=%d,cacheidx=%d, p->probeNode=%p",
            needSize, p->probeNodeIdx, p->probeCacheIdx, p->probeNode);
        if(NULL == p->probeNode)
            break;
        ...
        if(node->nLength >= needSize)
        {
            ...
            readSize += needSize;
            logd("[1]node->nLength=%d, needSize=%lld", node->nLength, needSize);
            break;
        }
        else
        {
            ...
            logv("[2]node->nLength=%d, needSize=%lld", node->nLength, needSize);
            c->pProbeHead = node->pNext;
            p->probeNode = NULL;
            p->probeNodePos = 0;
            p->probeNodeIdx++;
