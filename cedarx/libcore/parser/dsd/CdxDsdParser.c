...
static cdx_bool read_header(dsdiff_header *pHead, dsdfile *file) {
    cdx_uint8 buffer[12];
...
static cdx_bool read_chunk_header(dsdiff_chunk_header *head, dsdfile *file) {
    cdx_uint8 buffer[12];
...
static cdx_bool parse_prop_chunk(dsdfile *pFile, dsdiff_chunk_header *head) {
...
static cdx_bool read_fmt_chunk(fmt_chunk *pDsf_fmt, dsdfile *file) {
    cdx_uint8 buffer[52];
    cdx_uint8 *pPtr=buffer;
    if ((size_t)dsd_read_raw(buffer, sizeof(buffer), file) < sizeof(buffer))
        return CDX_FALSE;
...
static cdx_bool read_data_header(data_header *pDsf_datahead, dsdfile *file) {
    cdx_uint8 buffer[12];
    cdx_uint8 *pPtr=buffer;
...
static cdx_int32 aw_dsdiff_set_pos(dsdfile *file, cdx_uint32 mseconds, cdx_uint64 *offset) {
...
static cdx_int32 aw_dsf_set_pos(dsdfile *pFile, cdx_uint32 mseconds, cdx_uint64 *offset) {
