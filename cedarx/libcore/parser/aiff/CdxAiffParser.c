...
static int cdx_mov_read_chan(CdxStreamT *pb, int64_t size)
{
    ...
    CDX_LOGV("chan: layout=%u bitmap=%u num_descr=%u\n",
...
static int AiffInit(CdxParserT* parameter)
{
    ...
    {
        CDX_LOGW("LINE : %d. failed to open.",__LINE__);
        goto OPENFAILURE;
    }
    filesize -= 4;
...
got_sound:
    ...
    impl->mErrno = PSR_OK;
    CDX_LOGV("offset : %lld, Snd data end : %lld, dSndataOffset : %lld",
...
static cdx_int32 __AiffParserSeekTo(CdxParserT *parser, cdx_int64 timeUs, SeekModeType seekModeType)
{
    ...
    CDX_LOGV("nFrames_off : %lld", nFrames_off);
