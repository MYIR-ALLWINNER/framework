...
void CorrectAudioPackId(CdxMpgParserT *MpgParser, cdx_uint8 audIdx)
{
     ...
     flagMp2 = 0;//((dvdIfo->audioIfo.audioPackId[audIdx]&0x01e0)==0x01d0);//Always false, maybe a bug
     ...
     for(i=0; i<dvdIfo->audioIfo.audioWrongNum; i++)
     {
       if(dvdIfo->audioIfo.audioErrorPackId[i]==MPG_PRIVATE_STREAM_2)
       {
          ...
          newflagMp2 = 0;//((dvdIfo->audioIfo.audioErrorStrmId[i]&0x01e0)==0x01d0);
          //Always false, maybe a bug
