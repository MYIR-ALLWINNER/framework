...
void  ProbeAacAudio(CdxMpgParserT *MpgParser, cdx_uint8 *curPtr)
{
    ...
    #if 0
    //check the audio codec format is AAC_LC, the ADTS is the audio format AAC_LC
    adts_fixed_header()
    {
        syncword;                      12bits   0xfff, all bits must be 1
        ID;                             1 bit   mpeg version 0 for mpeg4, 1 for mpeg2
        layer;                          2 bits  always 00
        protection_absent;              1 bit
        profile;                        2 bit  0: main profile 1: Low cpmlexity profile
                                               2:scalable sampling Rate profile
                                               3: reserved
        sampling_frequency_index;       4 bits   0~15
        private_bit;                    1 bit
        channel_configuration;          3 bits   0~15
        original_copy;                  1 bit
        home;                           1 bit
    }
    #endif
    ...
    if(nextCode == 0xfff0)
    {
        ...
        if(adtsSampRateIdx > 15)
        {
            return;
        }

        if(adsChannelConfig > 15)
        {
            return;
        }
        MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].eCodecFormat =
