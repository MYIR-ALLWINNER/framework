        ...
        while(!lastblock)
        {
            ...
            switch(blocktype)
            {
                case 0:  //METADATA_BLOCK_STREAMINFO
                    ...
                    if (!impl->totalsamples) {
                        impl->ulNoTotalSamples = 1;
                    }
                    ...
                    CDX_LOGV("impl->ulDuration=%lldms", impl->ulDuration);
                    break;
...
SUCESS:
    ...
    /*
     * Maybe some flac have no totalsamples, we have to caculate it by audio frame,
     * and it's not very precise.
     */
    if (impl->ulNoTotalSamples) {
        CdxPacketT packet;
        do {
            //CDX_LOGD("CdxParserPrefetch:Start");
            memset(&packet, 0x00, sizeof(CdxPacketT));
            if(CdxParserPrefetch(&impl->base, &packet) != 0) {
                CDX_LOGD("CdxParserPrefetch != 0");
                break;
            }
            if (packet.type == CDX_MEDIA_AUDIO) {
                impl->totalsamples += impl->orgSampsPerFrm;
                //CDX_LOGD("impl->totalsamples :%lld", impl->totalsamples);
            }
            impl->file_offset += packet.length;
            CdxStreamSeek(impl->stream, impl->file_offset, SEEK_SET);
            //CDX_LOGD("CdxParserPrefetch:End");
        } while (1);
        impl->nowsamples = 0;
        impl->file_offset = impl->data_offset;
        impl->ulDuration = (cdx_int64)((impl->totalsamples*1000)/impl->ulSampleRate);
        CDX_LOGE("final impl->ulDuration=%lldms", impl->ulDuration);
    }
    CdxStreamSeek(cdxStream, impl->data_offset,SEEK_SET);
...
static cdx_int32 __FlacParserGetMediaInfo(CdxParserT *parser, CdxMediaInfoT *mediaInfo)
{
    ...
    //CDX_LOGD("impl->fileSize=%lld, impl->ulDuration=%lld", impl->fileSize, impl->ulDuration);
    //CDX_LOGD("mediaInfo->fileSize=%lld, cdxProgram->duration=%u", mediaInfo->fileSize, cdxProgram->duration);
    return CDX_SUCCESS;
}
