...
#define MIN_MP3_VALID_FRAME_COMBO 3
...
static float Resync(CdxStreamProbeDataT *p){
    ...
    if (inout_pos == 0) {
        ...
        for (;;) {
            ...
            len += 10;
            inout_pos += len;
            CDX_LOGV("skipped ID3 tag, new starting offset is %lld (0x%016llx)",
                 inout_pos, inout_pos);
            if(inout_pos != 0)
            {
                CDX_LOGV("this is ID3 first : %lld",inout_pos);
                return 0.;
            }
        }
    ...
    do {
        ...
        if (valid) {
            ...
            CDX_LOGV("no dice, no valid sequence of frames found. combo : %d", j);
        }
    ...
    if(valid)
    {
        CDX_LOGV("inout_pos : %lld, frame_combo : %d, probe len : %d",
...
// Any question -> chengkan@allwinnertech.com
    ...
    do {
        ...
        CDX_LOGV("found possible 1st frame at %lld (header = 0x%08x) mFrameSize %d, mBitRate : %d",
        ...
        for (j = 0; j < SYNCFRMNUM - 1; ++j) {
            ...
            CDX_LOGV("found subsequent frame #%d at %lld", j + 2, test_pos);
            CDX_LOGV("subsequent test_frame_size(%d), SR(%d), CH(%d), BITR(%d)",
                    test_frame_size, testSampleRate, testNumChannels, testBitRate);
            test_pos += test_frame_size;
            avgbitrate += testBitRate;
        }
        CDX_LOGV("==== Run for %d frame to calc avgbitrate ===", j+1);
...
static int CdxMp3Init(CdxParserT* Parameter)
{
    ...
    if(!CdxStreamIsNetStream(mp3->stream))//do not parse v1 tag in network stream...
    {
        ...
        CDX_LOGV("Mp3 start Pos : %lld", Pos);
...
static int CdxMp3ParserGetMediaInfo(CdxParserT *parser, CdxMediaInfoT *mediaInfo)
{
    ...
    {
        CDX_LOGV("mp3 version 1...");
        Id3BaseGetMetaData(mediaInfo, mp3->id3v1);
    }
    else
        CDX_LOGV("No id3 version 1 or id3 version 2 has been parsed, return none...");
    return ret;
}
