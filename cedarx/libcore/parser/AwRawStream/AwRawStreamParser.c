...
static int AwRawStreamParserControl(CdxParserT *parser, int cmd, void *param)
{
    ...
    switch(cmd)
    {
        ...
        case CDX_PSR_CMD_SWITCH_SUBTITLE:
            CDX_LOGW(" awts parser is not support switch stream yet!!!");
            break;
...
static int AwRawStreamProbeH264(cdx_char *p)
{
    ...
    if(code != 0)
    {
        CDX_LOGV(" Maybe this is not h264 stream. forbidden_zero_bit != 0  ");
        return 0;
    }
    type = p[0] & 0x1f;
    if(type <= 13 || type == 19)
    {
        CDX_LOGV(" Maybe this is h264 stream. nal unit type = %d", type);
        return 50;
    }
    else
    {
        CDX_LOGV(" This is not h264 stream. nal unit type = %d", type);
        return 0;
    }
    return 0;
}
...
static int AwRawStreamProbeH265(cdx_char *p)
{
    ...
    if(code != 0)
    {
        CDX_LOGV(" Maybe this is not h265 stream. forbidden_zero_bit != 0  ");
        return ret;
    }
    type = (p[0] & 0x7e) >> 1;
    if(type > 63)
    {
        CDX_LOGV(" Maybe this is not h265 stream. nal unit type > 63");
        return ret;
    }
    switch(type)
    {
        ...
        case 0x22: /*PPS*/
        {
            ...
            CDX_LOGV(" Parser probe result: This is h265 raw stream:%d! ",
                nNuhLayerId);
            ret = 50;
            break;
        }
