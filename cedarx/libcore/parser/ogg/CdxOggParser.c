...
static int ogg_new_stream(CdxOggParser *ogg, uint32_t serial)
{
    ...
    CDX_LOGV("serial=0x%x, idx=%d", serial, idx);
    return idx;
}
...
{
    CDX_LOGV("ogg_get_duration start");
    if(ogg->fileSize <= 0)
    {
...
_start:
    ...
    CDX_LOGV("junmp pos %lld, data offset %llu, real size %lld, dst frame %lld, total_frame %lld.",
                    jump_file_pos, ogg->data_offset, practicalSize, dst_frame, ogg->total_frame);
    do
    {
    reseek:
        ...
        CDX_LOGV("guess_frame=%lld", guess_frame);
        /*//在极端情况下，有死循环的风险
        if(guess_frame == 0)
        {
            if(ret < ogg->bufSize)
        ...
        if(guess_frame < dst_frame)
        {
            ...
            if(diff_frame > toleranceL)
            {
                ...
                if(jump_file_pos > ogg->fileSize)
                {
                    jump_file_pos -= diff_frame * practicalSize/ogg->total_frame;
                    CDX_LOGW("reset jump pos when beyond file size as %lld.", jump_file_pos);
                }
            }else
            {
                pos_find = 1;
...
static int SetMediaInfo(CdxOggParser *oggParser)
{
    ...
    CDX_LOGV("*********oggParser->durationUs=%llu", oggParser->durationUs);
    int i, j=0;
    struct ogg_stream *os;
...
{
    CDX_LOGV("OggParserForceStop start");
    CdxOggParser *oggParser = (CdxOggParser*)parser;
    pthread_mutex_lock(&oggParser->statusLock);
    ...
    oggParser->mErrno = PSR_USER_CANCEL;
    oggParser->status = CDX_PSR_IDLE;
    CDX_LOGV("OggParserForceStop end");
    return 0;
}
...
cdx_int32 OggParserSeekTo(CdxParserT *parser, cdx_int64 timeUs, SeekModeType seekModeType)
{
    ...
    CDX_LOGV("OggParserSeekTo start, timeUs = %lld", timeUs);
    CdxOggParser *oggParser = (CdxOggParser *)parser;
    oggParser->mErrno = PSR_OK;
    oggParser->status = CDX_PSR_IDLE;
...
cdx_uint32 OggParserProbe(CdxStreamProbeDataT *probeData)
{
    ...
    {
        CDX_LOGV("It is not ogg version 0.");
        return 0;
    }
    return 100;
}
