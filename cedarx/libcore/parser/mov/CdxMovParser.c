...
#if SAVE_VIDEO
        if(pkt->type == CDX_MEDIA_VIDEO && tmpMovPsr->fp)
        {
            char buf[4096];
            fprintf(tmpMovPsr->fp, "\n");
            sprintf(buf, "%s%llx", "offset:", c->chunk_info.offset);
...
static cdx_int32 __CdxMovParserGetMediaInfo(CdxParserT *parser, CdxMediaInfoT * pMediaInfo)
{
    ...
    for(i=0; i<c->nb_streams; i++)
    {
        ...
        if(st->codec.codecType == CODEC_TYPE_VIDEO && !st->unsurpoort)
        {
            ...
            {
                logw("get (%d)th video info fail, cause max num = %d", i,MAX_STREAM_NUM);
            }
        ...
        else if(st->codec.codecType == CODEC_TYPE_AUDIO && !st->unsurpoort)
        {
            ...
            {
                logw("get (%d)th audio info fail, cause max num = %d", i,MAX_STREAM_NUM);
            }
        ...
        else if(st->codec.codecType == CODEC_TYPE_SUBTITLE)
        {
            ...
            {
                logw("get (%d)th subtitle info fail, cause max num = %d", i,MAX_STREAM_NUM);
            }
...
static CdxParserT *__CdxMovParserOpen(CdxStreamT *stream, cdx_uint32 flags)
{
    ...
    if(!tmpMovPsr->fp)
        logw("saveVideo:open /data/camera/video.es fail!");
#endif
