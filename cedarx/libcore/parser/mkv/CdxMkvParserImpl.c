...
cdx_int32 matroska_add_stream (MatroskaDemuxContext *matroska)
{
        ...
        switch (id)
        {
                ...
                while (res == 0)
                {
                    ...
                    switch (id)
                    {
                        ...
                        case CDX_MATROSKA_ID_VIDEOFRAMERATE:
                        {
                            ...
                            CDX_LOGV("--- video framerate = %d", videotrack->frame_rate);
            ...
            case CDX_MATROSKA_ID_TRACKLANGUAGE:
            {
                ...
                if (CDX_ISO_639_LANGS[i].eng_name == NULL)
                {
                    ...
                    {
                        if (strcmp(text, "und"))
                            CDX_LOGV("unrecognized language: %s", text);
    ...
    if(track_name)
    {
        ...
        CDX_LOGV("-- name_len = %d, language = %s", name_len, track->language);
                    ...
                    case CDX_MATROSKA_ID_TAGS:
                    {
                        ...
                        switch (id)
                        {
                            ...
                            case CDX_MATROSKA_ID_TRACKS:
                                ...
                                if (matroska->num_tracks || !res || FEOF(matroska)) {
                                    matroska->track_parsed = 1;
                                    res = 0;
                                }
                                break;
                        }
                        ...
                        unsigned char **extradata, int *extradata_size)
{
    ...
    CDX_LOGV("len1 = %x, len2:%x", len1, len2);
...
cdx_int32 matroska_read_header (struct CdxMkvParser *p)
{
    ...
    CDX_LOGV("segment strat pos = %lld", matroska->segment_start);
    ...
    while (res == 0)
    {
        ...
        switch (id)
        {
            ...
            {
                CDX_LOGV("track");
                if(!matroska->track_parsed) {
                    res = ebml_read_master(matroska, &id);
                    if (res < 0)
                        break;
                    res = matroska_parse_tracks(matroska);
                } else {
                    CDX_LOGD("track are already parsed while parsing seekhead");
                    res = ebml_read_skip(matroska);
                }
                break;
            }
...
#define max_as_search 24
        ...
        for (i = 0; i < matroska->num_tracks; i++)
        {
                ...
                (track->codec_priv != NULL))
            {
                ...
                CDX_LOGV("--- fourcc=%x", vtrack->fourcc);
    ...
    if (p->hasAudio == 0 && p->hasVideo == 0)
    {
        loge("has no video and no audio");
        return -1;
    }
        ...
        switch (id)
        {
            ...
            case CDX_MATROSKA_ID_CLUSTERTIMECODE:
            {
                   ...
                   (matroska->segment_duration * AV_TIME_BASE))
                {
                    ...
                    CDX_LOGW("care! the cluster time is not start from 0");
                }
                break;
            }
                                 ...
                                 cdx_int32 ms)
{
    ...
    if(next_index == 0)
    {
        ...
        {
            CDX_LOGV("index = %d, st->nb_index_entries = %d", index, st->nb_index_entries);
            return -1;
        }
...
static int MkvGetClusterTimeCode(struct CdxMkvParser *p, cdx_uint64* num)
{
    ...
    cdx_uint32 read;
    cdx_uint64 rlength;
    CDX_LOGV("--- offset: %llx", CdxStreamTell(matroska->fp));

    while (res == 0)
    {
        read = ebml_read_num(matroska, 4, &total);
        id = (cdx_uint32)(total | (1 << (read * 7)));
        CDX_LOGV("--- id = %x, total = %llx, read = %d, pos = %llx",
        ...
        switch (id)
        {
            ...
            case CDX_MATROSKA_ID_CLUSTERTIMECODE:
            {
                CDX_LOGV("--- TimeCode offset = %llx", CdxStreamTell(matroska->fp));
                int n=0;
                cdx_uint8  t[16];
                ...
                *num = 0;
                ...
                CDX_LOGV("--- timecode length = %lld, offset = %llx",
...
static int MkvFindCluster(struct CdxMkvParser *p, cdx_uint64* num)
{
    ...
    CDX_LOGV("***** find cluster %llx", CdxStreamTell(matroska->fp));
    ...
    *num = 0;
    ...
    while (n++ < size)
        *num = (*num << 8) | t[n-1];
 #endif
    CDX_LOGV("cluster length = %llx", *num);
...
static int MkvEstimateBR(struct CdxMkvParser *p)
{
    ...
    cdx_int64 wholeClusterTime = (clusterTime2 - clusterTime) * matroska->time_scale / AV_TIME_BASE;
    if(matroska->segment_duration * 2 < wholeClusterTime)
        matroska->segment_duration = wholeClusterTime +
            (matroska->fileEndPst - offset2)/matroska->bitrate;
    CDX_LOGV("-- bitrate = %lld", matroska->bitrate);
    return 0;
}
...
static int MkvResync(struct CdxMkvParser *p)
{
    ...
    CDX_LOGV("id = %x, pos = %lld(%llx)", id, tell, tell);
...
int CdxMatroskaOpen(struct CdxMkvParser *p)
{
    ...
    {
        CDX_LOGW("can not get the stream size ");
        matroska->fileEndPst = 0x7fffffffffffff;
    }
    CDX_LOGV("file size = %lld", matroska->fileEndPst);
    ...
    if(p->hasVideo)
    {
        ...
        switch (track->codec_id)
        {
            ...
            case CODEC_ID_H265:
            {
                CDX_LOGV("video bitstream type: H265\n");
                p->vFormat.pCodecSpecificData =
                        (void *)matroska->streams[p->VideoStreamIndex]->extradata;
                p->vFormat.nCodecSpecificDataLen =
                        matroska->streams[p->VideoStreamIndex]->extradata_size;
            ...
            {
                CDX_LOGV("video bitstream type: MPEG1\n");
                p->vFormat.pCodecSpecificData =
                        (void *)matroska->streams[p->VideoStreamIndex]->extradata;
                p->vFormat.nCodecSpecificDataLen =
                        matroska->streams[p->VideoStreamIndex]->extradata_size;
            ...
            case CODEC_ID_RV40:
            {
                CDX_LOGV("video bitstream type: RMVB89\n");
                matroska->video_direct_copy = 0;
            ...
            default:
            {
                logw("unknown codec 0x%x", track->codec_id);
            ...
            if (track->type == MATROSKA_TRACK_TYPE_AUDIO)
            {
                ...
                switch(track->codec_id)
                {
                    ...
                    case CODEC_ID_WMAPRO:
                        ...
                        aFormat->eCodecFormat     = AUDIO_CODEC_FORMAT_WMA_STANDARD;
                        aFormat->eSubCodecFormat = audiotrack->tag;
                        CDX_LOGV("---- CODEC_ID_WMAPRO codec_tag = %d", audiotrack->tag);
                        aFormat->nCodecSpecificDataLen =
                                matroska->streams[stream_index]->extradata_size;
                        aFormat->pCodecSpecificData =
                                matroska->streams[stream_index]->extradata;
                    ...
                    case CODEC_ID_FLAC:
                        ...
                        break;
                    case CODEC_ID_OPUS:
                        CDX_LOGV("audio bitstream type: CODEC_ID_OPUS!\n");
...
int CdxMatroskaSeek(struct CdxMkvParser *p, cdx_int64  timeUs)
{
    ...
    CDX_LOGV("============= seek to: %lldms  total: %lld\n", timeUs, matroska->segment_duration);
