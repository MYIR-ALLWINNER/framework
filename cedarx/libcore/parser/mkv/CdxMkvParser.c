...
{
    CDX_LOGV("--- __CdxMkvParserClose");
    int ret;
    struct CdxMkvParser* impl = (struct CdxMkvParser*)parser;
    if(!impl)
    {
        CDX_LOGE("impl is NULL.");
        return -1;
    }
    impl->exitFlag = 1;
...
static cdx_int32 __CdxMkvParserGetMediaInfo(CdxParserT *parser, CdxMediaInfoT * pMediaInfo)
{
    ...
    CDX_LOGV("----mkv seekable = %d", pMediaInfo->bSeekable);
    ...
    CDX_LOGV("*********** video stream ************");
    CDX_LOGV("****eCodecFormat:           %x", impl->vFormat.eCodecFormat);
    CDX_LOGV("****nFrameRate:             %d", impl->vFormat.nFrameRate);
    CDX_LOGV("****nWidth:                 %d", impl->vFormat.nWidth);
    CDX_LOGV("****nHeight:                %d", impl->vFormat.nHeight);
    CDX_LOGV("****nCodecSpecificDataLen:  %d", impl->vFormat.nCodecSpecificDataLen);
    CDX_LOGV("************************************");
    ...
    for(i=0; i<impl->nhasAudio; i++)
    {
        ...
        CDX_LOGV("***********audio stream %d************", i);
        CDX_LOGV("****language:               %s", pMediaInfo->program[0].audio[i].strLang);
        CDX_LOGV("****eCodecFormat:           %d", impl->aFormat_arry[i].eCodecFormat);
        CDX_LOGV("****eSubCodecFormat:        %d", impl->aFormat_arry[i].eSubCodecFormat);
        CDX_LOGV("****nChannelNum:            %d", impl->aFormat_arry[i].nChannelNum);
        CDX_LOGV("****nAvgBitrate:            %d", impl->aFormat_arry[i].nAvgBitrate);
        CDX_LOGV("****nMaxBitRate:            %d", impl->aFormat_arry[i].nMaxBitRate);
        CDX_LOGV("****nSampleRate:            %d", impl->aFormat_arry[i].nSampleRate);
        CDX_LOGV("****nBlockAlign:            %d", impl->aFormat_arry[i].nBlockAlign);
        CDX_LOGV("****nCodecSpecificDataLen:  %d", impl->aFormat_arry[i].nCodecSpecificDataLen);
        CDX_LOGV("************************************");
    }
    ...
    for(i=0; i<pMediaInfo->program[0].subtitleNum; i++)
    {
        ...
        CDX_LOGV("***********subtitle %d****************", i);
        CDX_LOGV("****streamindex:   %d", pMediaInfo->program[0].subtitle[i].nStreamIndex);
        CDX_LOGV("****width:         %d", pMediaInfo->program[0].subtitle[i].nReferenceVideoWidth);
        CDX_LOGV("****height:        %d", pMediaInfo->program[0].subtitle[i].nReferenceVideoHeight);
        CDX_LOGV("****TextFormat:    %d", pMediaInfo->program[0].subtitle[i].eTextFormat);
        CDX_LOGV("****eCodecFormat:  %x", pMediaInfo->program[0].subtitle[i].eCodecFormat);
        CDX_LOGV("****Language:      %s", pMediaInfo->program[0].subtitle[i].strLang);
        CDX_LOGV("****extradataSize: %d", matroska->streams[stream_idx]->extradata_size);
        CDX_LOGV("***************************************");
...
cdx_int32 __CdxMkvParserSeekTo(CdxParserT *parser, cdx_int64  timeUs, SeekModeType seekModeType)
{
    ...
    {
        CDX_LOGV("EOS, timeUs = %lld, duration = %lld", timeUs/1000, matroska->segment_duration);
        impl->mErrno = PSR_EOS;
        return 0;
    }
...
static CdxParserT *__CdxMkvParserOpen(CdxStreamT *stream, cdx_uint32 flag)
{
    ...
    CDX_LOGV("mkv parser open stream = %p", stream);
