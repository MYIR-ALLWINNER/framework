...
static cdx_int32 ApeSeek(CdxStreamT *stream,cdx_int64 offset,cdx_int32 whence,cdx_int64 baseptr)
{
    if(whence == SEEK_CUR){
        return CdxStreamSeek(stream,offset,SEEK_CUR);
    }else if(whence == SEEK_SET){
        return CdxStreamSeek(stream,offset + baseptr,SEEK_SET);
    }
    CDX_LOGE("Ape seek error!!");
    return -1;
}
...
static int CdxApeInit(CdxParserT* Parameter)
{
    ...
    cdx_int64 pts = 0;
    cdx_int64  file_size;
    cdx_int32 final_size = 0;

    pApe = (APEParserImpl *)Parameter;

    pApe->baseptr = CdxStreamTell(pApe->stream);
    ...
    if(pApe->fileversion >= 3980)
    {
        ...
        if (pApe->descriptorlength > 52)
            ApeSeek(pApe->stream, pApe->descriptorlength - 52, SEEK_CUR,0);
    ...
    else
    {
        ...
        {
            ApeSeek(pApe->stream, 4, SEEK_CUR,0); /* Skip the peak level */
            pApe->headerlength += 4;
        }
    ...
    file_size = CdxStreamSize(pApe->stream);
    if (file_size > 0) {
        final_size = file_size - pApe->frames[pApe->totalframes - 1].pos -
                     pApe->wavtaillength;
        final_size -= final_size & 3;
    }
    if (file_size <= 0 || final_size <= 0)
        final_size = pApe->finalframeblocks * 8;
    pApe->frames[pApe->totalframes - 1].size = final_size;
    ...
    if(pApe->fileversion < 3810)
    {
        for(i=0; i<pApe->totalframes; i++)
        {
            if(i < pApe->totalframes-1 && pApe->bittable[i+1])
             ...
             pApe->frames[i].skip = pApe->frames[i].skip << 3;
    ...
    pApe->extradata[5]=  pApe->frames[pApe->totalframes - 1].size & 0xff;
    pApe->extradata[6]= (pApe->frames[pApe->totalframes - 1].size>>8) & 0xff;
    pApe->extradata[7]= (pApe->frames[pApe->totalframes - 1].size>>16) & 0xff;
...
static int CdxApeParserRead(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    ret = ApeSeek(pApe->stream, nreadpos, SEEK_SET,pApe->baseptr);
    if (ret < 0) {
