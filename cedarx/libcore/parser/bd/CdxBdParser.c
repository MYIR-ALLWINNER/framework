...
//    CDX_LOGD("bdParser->prefetchType = %d", bdParser->prefetchType);
    ...
    //CDX_LOGW("[prefetch] type:%d, pkt->pts:%lld, pkt->length:%d, pkt->streamIndex:%d, bdParser->streamPts[1]:%lld, bdParser->streamPts[0]:%lld!!, bdParser->prefetchType:%d", 
    //       pkt->type, pkt->pts, pkt->length, pkt->streamIndex,
    ...
    {
        CDX_LOGW("[fix] maybe BD_BASE pts jump, streamPts[0]:%lld, streamPts[1]:%lld",
            bdParser->streamPts[0],bdParser->streamPts[1]);
    ...
    {
        CDX_LOGW("[fix] maybe the stream0 has something wrong, streamPts[0]:%lld, streamPts[1]:%lld!!",
            bdParser->streamPts[0], bdParser->streamPts[1]);
        bdParser->streamPts[0] = 0;
    }
...
static cdx_int32 BdParserRead(CdxParserT *parser, CdxPacketT *pkt)
{
   ...
   /*
    ...
    //CDX_LOGW("[read] type:%d, pkt->pts:%lld, pkt->length:%d, pkt->streamIndex:%d, bdParser->streamPts[1]:%lld, bdParser->streamPts[0]:%lld!!, bdParser->prefetchType:%d", 
    //       pkt->type, pkt->pts, pkt->length, pkt->streamIndex, 
    //       bdParser->streamPts[1], bdParser->streamPts[0], bdParser->prefetchType);
    bdParser->status = CDX_PSR_IDLE;
    return ret;
...
//    char *tmp = strstr(bdParser->originalUrl, "BDMV/");
    ...
    CdxListForEachEntrySafe(playlist, playlist1, &bdParser->playlists, node)
    {
        ...
        {
            if (tmpDurationUs > playlist->durationUs)
                break;
            bdParser->hasMvcVideo = 1;
            bdParser->curPlaylist = playlist;
            tmpDurationUs = playlist->durationUs;
        }
