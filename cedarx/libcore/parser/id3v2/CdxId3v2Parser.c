...
static int Id3v2Init(CdxParserT *id3_impl)
{
    ...
    {
        CDX_LOGV("Parser id3 success");
    }
    else
    {
        CDX_LOGV("Id3_v2 parser get fail? Joking me....");
        goto OPENFAILURE;
    }

    CdxStreamProbeDataT *probeData = CdxStreamGetProbeData(impl->stream);

    CDX_LOGV("From(%lld) skip to post id3 location(%lld), totally skip (%lld + %d)...",
impl->cur_id3_offset, impl->cur_id3_offset + offset, probeData->sync_pos, impl->id3v2->mRawSize);
    CDX_LOGV("Now actually offset %lld", CdxStreamTell(impl->stream));
    ...
    CDX_LOGV("impl->newurl(%s), impl->file_offset(%lld)", impl->newurl, impl->cur_id3_offset);
    impl->cdxDataSource.uri = impl->newurl;

    if ((int)probeData->len > offset + PROBE_PREOTECTION)
    {
        probeData->len -= offset;
        CDX_LOGV("Id3 probe data is enough at absolutive pos(%lld), reuse(sync_pos(%lld), skip(%d),\
 left(%d))", impl->fdoffset, probeData->sync_pos, offset, probeData->len);
    ...
    CDX_LOGV("Id3 probe data is not enough at absolutive pos(%lld), probe has(%d), need skip(%d)\
...
static cdx_int32 __Id3v2ParserControl(CdxParserT *parser, cdx_int32 cmd, void *param)
{
    ...
    switch (cmd)
    {
        ...
        case CDX_PSR_CMD_CLR_FORCESTOP:
            ...
            break;
        default :
            CDX_LOGV("Default success to child processing...");
...
static cdx_int32 __Id3v2ParserGetMediaInfo(CdxParserT *parser, CdxMediaInfoT *mediaInfo)
{
    ...
    if(!(impl->id3Attributes & DO_NOT_EXTRACT_ID3_METADATA))
    {
        ...
        {
            CDX_LOGV("id3v2 has vaild parsed...");
...
static cdx_int32 __Id3v2ParserClose(CdxParserT *parser)
{
    ...
    if(impl->stream && !(impl->id3Attributes & SUCCESS_STREAM_TO_CHILD)){
        CDX_LOGV("id3v2 close stream it self...");
        CdxStreamClose(impl->stream);
        impl->stream = NULL;
    }
    else
    {
        CDX_LOGV("id3v2 success stream to its child, child close stream!");
    }
...
static cdx_uint32 __Id3v2ParserProbe(CdxStreamProbeDataT *probeData)
{
    ...
    CDX_LOGV("Id3v2 probe score : %d, offset : %d", score, i);
