    ...
    for (;;)
    {
        ...
        if (!memcmp(&guid, &file_header, sizeof(AsfGuid)))
        {
            ...
            if(ctx->hdr.play_time)
               ...
               ctx->durationMs = 0;
            CDX_LOGV("++++ durationMs: %lld", ctx->durationMs);
        }
    ...
    {    //we must enhance it
      ...
      if(ctx->nb_packets != 0xffffffff)
      {
        ...
        while (1)
        {
            ...
            {
                CDX_LOGW("invalid time (%d, %lld), maybe data error", curr_seconds, ctx->durationMs);
                //return -1;
                startpkt++;
                continue;
            }
...
int AsfPsrCoreIoctrl(AsfContextT *ctx, cdx_uint32 uCmd, void* uParam)
{
    switch(uCmd)
    {
    case CDX_MEDIA_STATUS_PLAY:
