...
static int PlsParse(char *data, cdx_uint32 size, PlaylistT **P, const char *baseURI)
{
    ...
    while(offset < size)
    {
        ...
        {
            CDX_LOGV("pls file version: %s", lineKey);
            free(line);
            line = NULL;
            offset = offsetLF + 1;
            continue;
        ...
        {
            CDX_LOGV("pls should have %d entries", atoi(lineValue));
            free(line);
            line = NULL;
            offset = offsetLF + 1;
            continue;
        ...
        {
            CDX_LOGV("couldn't find number of items");
            free(line);
            line = NULL;
            offset = offsetLF + 1;
            continue;
...
static cdx_int32 __PlsParserPrefetch(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    if(ret < 0)
    {
        ...
        if(mErrno == PSR_EOS)
        {
            ...
            {
                CDX_LOGV("plsParser->status = PSR_EOS");
...
cdx_int32 PlsParserForceStop(CdxParserT *parser)
{
    ...
    plsParser->status = CDX_PLS_IDLE;
    pthread_mutex_unlock(&plsParser->statusLock);
    CDX_LOGV("PlsParserForceStop end");
    return 0;
...
int __PlsParserInit(CdxParserT *parser)
{
    ...
    CDX_LOGV("duration: %lld ms, %u ms", pls->durationUs / 1000, pMediaInfo->program[0].duration);
