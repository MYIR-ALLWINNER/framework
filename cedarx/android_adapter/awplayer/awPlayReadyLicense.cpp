...
{
    DRM_RESULT dr = DRM_SUCCESS;
    logv("%s %d, dr: %08x", __func__, __LINE__, dr);
    ...
    logv("%s %d result: %08x", __func__, __LINE__, oLicResponse.m_dwResult);
    ...
    case FUN_PROCESS_LICENSE:
        {
            ...
            logv("wrmheader=%s", wrmheader.string());
            logv("la_url=%s", la_url.string());
