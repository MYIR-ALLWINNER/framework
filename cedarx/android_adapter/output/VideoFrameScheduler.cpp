...
void VideoFrameScheduler::vfsUpdateVsync() {
...
void VideoFrameScheduler::vfsInit(float videoFps) {
    vfsUpdateVsync();
    mLastVsyncTime = -1;
    mTimeCorrection = 0;
    mPll.reset(videoFps);
...
void VideoFrameScheduler::vfsRestart() {
    mLastVsyncTime = -1;
    mTimeCorrection = 0;
...
nsecs_t VideoFrameScheduler::vfsGetVsyncPeriod() {
    if (mVsyncPeriod > 0) {
        return mVsyncPeriod;
    }
...
float VideoFrameScheduler::vfsGetFrameRate() {
...
nsecs_t VideoFrameScheduler::vfsSchedule(nsecs_t renderTime) {
...
void VideoFrameScheduler::vfsRelease() {
    mComposer.clear();
}
