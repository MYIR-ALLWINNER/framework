...
int XPlayerSetVideoPlaybackSpeed(XPlayer* p,float speed)
{
    ...
    xrate.mSpeed = speed;
    xrate.mPitch= 0.0;
    xrate.mStretchMode= XAUDIO_TIMESTRETCH_STRETCH_DEFAULT;
    xrate.mFallbackMode= XAUDIO_TIMESTRETCH_FALLBACK_DEFAULT;

    if(xrate.mSpeed == 1)//do not discard audio when speed = 1.
        PlayerSetDiscardAudio(p->mPlayer, 0);
    else
    {
        PlayerSetDiscardAudio(p->mPlayer, 1);
        logw("discard audio for set play speed in linux ");
    }

    XPlayerSetPlaybackSettings(p,&xrate);
    return 0;
}
