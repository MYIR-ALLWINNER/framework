...
int XRetrieverSetDataSource(XRetriever* v, const char* pUrl, const CdxKeyedVectorT* pHeaders)
{
    ...
    if(!p->mParser)
    {
        ...
        return NO_PARSER;//retriever can be created but all info would be NULL.
    }
...
XVideoFrame *XRetrieverGetFrameAtTime(XRetriever* v, int64_t timeUs, int option)
{
    ...
    int64_t first_pts;
    int first_flag = 0;
    int64_t nStartTime = GetCurrentTime();
    ...
    do
    {
        ...
        if((GetCurrentTime()-nStartTime) > MAX_TIME_TO_GET_A_FRAME)
        {
        ...
        if(ret == VDECODE_RESULT_FRAME_DECODED || ret == VDECODE_RESULT_KEYFRAME_DECODED)
        {
            ...
            if(pPicture != NULL)
            {
                ...
                if(bEncoderInited == 0)
                {
                    ...
                    logv("w:%d, h:%d, stride:%d", sBaseEncConfig.nInputWidth,
                            sBaseEncConfig.nInputHeight, pPicture->nLineStride);
                    logv("nRightOffset:%d, nLeftOffset:%d, nBottomOffset:%d, nTopOffset:%d",
...
int XRetrieverGetMetaData(XRetriever* v, int type, void* pVal)
{
    ...
    switch(type)
    {
        ...
        {
            int *w = (int*)pVal;
            logv("w: %d", p->mMediaInfo.program[0].video[0].nWidth);
        ...
        {
            int *h = (int*)pVal;
            logv("h: %d", p->mMediaInfo.program[0].video[0].nHeight);
