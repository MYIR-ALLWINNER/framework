...
#endif
{
	...
	for (i=0; i<sparse_header.total_chunks; i++) {
		...
		switch (chunk_header.chunk_type) {
			...
			if (chunk_header.total_sz != (sparse_header.chunk_hdr_sz +
				...
				fprintf(stderr, "Bogus chunk size for chunk %u, type Raw\n", i);
				return (-1);
			}
			...
			if (chunk_header.total_sz != (sparse_header.chunk_hdr_sz + sizeof(u32)) ) {
				fprintf(stderr, "Bogus chunk size for chunk %u, type Fill\n", i);
				return (-1);
			}
			...
			if (chunk_header.total_sz != sparse_header.chunk_hdr_sz) {
				fprintf(stderr, "Bogus chunk size for chunk %u, type Dont Care\n", i);
				return (-1);
			}
	...
	if (sparse_header.total_blks != total_blocks) {
		fprintf(stderr, "Wrote %u blocks, expected to write %u blocks\n",
