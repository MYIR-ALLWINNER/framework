...
static int read_frame(int mode)
{
	...
	if (save_flag == 0) {
		...
		disp_show(gCameraDmafd[buf.index],0,0,SCREEN_W,SCREEN_H);
#endif
	} else {
		count = 0;
	}
...
static int req_frame_buffers(void)
{
	...
	for (n_buffers = 0; n_buffers < req.count; ++n_buffers) {
		...
		case V4L2_MEMORY_DMABUF:
			for (int j = 0; j < nplanes; j++) {
				...
				gMem.size = (unsigned int)buffers[n_buffers].length[j];
				int ret = allocAlloc(MEM_TYPE_CDX_NEW, &gMem, NULL);
				if(ret < 0){
					printf("allocAlloc failed.\n");
					return -1;
				}

				buffers[n_buffers].phy_addr[j]= (void *)gMem.phy;
				buffers[n_buffers].start[j]= (void *)gMem.vir;
				...
				gCameraDmafd[n_buffers] = gMem.ion_buffer.fd_data.aw_fd;
				// printf("dbg1 req gCameraDmafd[%d]=%d buffers[%d].start[%d]=0x%x\n", 
				// 	n_buffers, gCameraDmafd[n_buffers],
				// 	n_buffers,j,buffers[n_buffers].start[j]);
			}
			break;
		}
	}
	...
	for (i = 0; i < n_buffers; ++i) {
		...
		if (-1 == ioctl(fd, VIDIOC_QBUF, &buf)) {
			...
			free(buf.m.planes);
			allocFree(MEM_TYPE_CDX_NEW,(dma_mem_des_t *)buffers[i].phy_addr[0],NULL);
			return -1;
		}
		free(buf.m.planes);
//		allocFree(MEM_TYPE_CDX_NEW,(void *)buffers[i].phy_addr[0],NULL);
	}
	return 0;
}
	...
	for (i = 0; i < n_buffers; ++i) {
		for (j = 0; j < nplanes; j++)
			if((V4L2_MEMORY_USERPTR == memtype )||(V4L2_MEMORY_DMABUF == memtype )){
				allocFree(MEM_TYPE_CDX_NEW,(dma_mem_des_t *)buffers[i].phy_addr[j],NULL);
				buffers[i].start[j] = NULL;
				buffers[i].phy_addr[j] = NULL;
...
{
	printf("usrptr_csi_test version:%s\n", MODULE_VERSION);
	int i, test_cnt = 1;
	int sel = 0;
	int width = 640;
	int height = 480;
