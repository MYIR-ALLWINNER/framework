/*
 * @Description: This file is used to test the functions of the G2D module 
 *               such as inversion, scaling, stitching, splitting, and so on.
 * @Version: 1.0
 * @Author: tangjianxiong
 * @Date: 2021-07-28 15:21:54
 * @LastEditors: tangjianxiong
 * @LastEditTime: 2021-08-03 11:02:16
 * @FilePath: \sdk_demo\G2dDemo\main.cpp
...
/* RGBA 90° convert */
...
#else
		/* Pass buffer with physical address */
	...
	case 1:
	{
		...
		gettimeofday(&t2, NULL);
		printf("================RGBA scaler use time =%lld================= \n", int64_t(t2.tv_sec) * 1000000000LL + int64_t(t2.tv_usec) * 1000LL - (int64_t(t1.tv_sec) * 1000000000LL + int64_t(t1.tv_usec) * 1000LL));
	...
	case 7:
		/* rgba to yuv */
		{
			...
			stRECT src_rect = {0, 0, src_width, src_height};

			/* output param */
			int dst_width = 1024;
			int dst_height = 600;
			int dst_format = (int)G2D_FORMAT_YUV420UVC_V1U1V0U0;
			int dst_size = src_width * src_height * 4;
			stRECT dst_rect = {0, 0, dst_width, dst_height};
			/* commond flag */
			int flag = (int)G2D_ROT_0;

			/* alloc input buffer */
			ReadPicFileContent(pRgbaInPicPath, &m_DispMemOps0, src_size);
			printf("alloc m_DispMemOps0.ion_buffer.fd_data.aw_fd=%d\n", m_DispMemOps0.ion_buffer.fd_data.aw_fd);
			/* reroll back */
			...
			printf("================== start RGBA scaler =============\n");
			gettimeofday(&t1, NULL);
			int ret = g2dClipByFd(g2dfd, src_fd, flag, src_format, src_width, src_height, 0, 0, src_width, src_height, dst_fd, dst_format, dst_width, dst_height, 0, 0, dst_width, dst_height);
			gettimeofday(&t2, NULL);
			printf("================RGBA scaler use time =%lld========= \n", int64_t(t2.tv_sec) * 1000000000LL + int64_t(t2.tv_usec) * 1000LL - (int64_t(t1.tv_sec) * 1000000000LL + int64_t(t1.tv_usec) * 1000LL));
			WritePicFileContent(pRgbaOutPicPath, &m_DispMemOps, dst_size);
			g2dUnit(g2dfd);
