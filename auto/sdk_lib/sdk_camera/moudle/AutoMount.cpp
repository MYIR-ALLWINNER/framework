    ...
    if (sdlen == 0) {
        ...
        if (dir != NULL) {
            ...
            while ((p = readdir(dir)) != NULL) {
                ...
                ALOGV("check sdcard 0  %s",p->d_name);

                if (p->d_name[0] != '.') {
                    i++;
                    ALOGV("check sdcard  %s ",p->d_name);
        ...
        for (i = 0; i < sdlen; i++) {
            ...
            if (!isMounted(buf)) {
                ALOGV("mount sdcard m %s",sdbuf[i]);
    ...
    if (action == NetlinkEvent::NlActionAdd) {
        ...
        if (!strcmp(devtype, "disk")) {
            ...
            if (rmret < 0) {
                ...
                ALOGV("mount real path is %s \r", real_path);
            }
            for (int i = 0; i < 6; i++) {
        ...
        if (!strcmp(devtype, "disk")) {
            ...
            if (tmp) {
                ...
                const char *devname = evt->findParam("DEVNAME");
                char buf[300];
                ALOGV("AutoMountCallback remove %s ",devname);
...
void enhanceUsbStability(NetlinkEvent *evt)
{
    ...
    if (tmp && !strncmp(tmp, "sd", 2)) {
       ...
       if ((fp = fopen(path, "r+ "))) {
           ...
           ALOGD("Success to write %d to %s ", msecs, path);
           fclose(fp);
       } else {
