...
void RdyRecord(bool bRdy)
{
	...
	{
		pdvr->startRecord();
		pdvr->SetCurRecordStat(RecordStat_NORMAL);
	}else
	{
		pdvr->stopRecord();
		pdvr->SetCurRecordStat(RecordStat_STOPCAR);
	}
#ifdef ANTHOR_CAMERA
	if(bRdy)
	{
		pdvr1->startRecord();
		pdvr1->SetCurRecordStat(RecordStat_NORMAL);
	}else
	{
		pdvr1->stopRecord();
		pdvr1->SetCurRecordStat(RecordStat_STOPCAR);
	}
#endif
#endif
