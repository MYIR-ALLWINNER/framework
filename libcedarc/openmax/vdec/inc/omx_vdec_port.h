 ...
 *
 *    Description: the function of ports(in port and out port),
 *                        called by omx_vdec and omx_vdec_decoder
 *
 *        Version:  1.0
 ...
 *                     The size of the array is set dynamically
 *                     using the nBufferCountActual value
 *                     send by the client. The order will change.
 *   nListEnd        - Marker to the boundary of the array.
                       This points to the last index of the pBufHdr array.
 *   nSizeOfList     - Count of valid data in the list.
 *   nAllocSize      - Size of the allocated list.
 *                     This is equal to (nListEnd + 1) in most of
 *                   the times. When the list is freed this is decremented and at that
 *                     time the value is not equal to (nListEnd + 1).
 *                   This is because
 *                     the list is not freed from the end and hence we cannot decrement
 *                     nListEnd each time we free an element in the list.
 *                   When nAllocSize is zero, the list is completely freed and
 *                     the other paramaters of the list are initialized.
 *                   If the client crashes before freeing up the buffers,
 *                     this parameter is checked (for nonzero value) to see
 *                      if there are still elements on the list.
 *                   If yes, then the remaining elements are freed.
 *    nWritePos     - The position where the next buffer would be written.
                      The value is incremented after the write.
 *                    It is wrapped around when it is greater than nListEnd.
 *    nReadPos      - The position from where the next buffer would be read.
 *                    The value is incremented after the read.
 *                    It is wrapped around when it is greater than nListEnd.
 *    eDir          - Type of BufferList.
 *                    OMX_DirInput  =  Input  Buffer List
 *                    OMX_DirOutput  =  Output Buffer List
 *
 *   pBufArr   -is not the pBufHdrList,
 *              the order of this array is fixed when allocting, not change
 *
