...
static int SecureAllocOpen2(void* veops, void* veself)
{
    ...
    pthread_mutex_init(&gAdapterMutex, NULL);
...
static void* SecureAllocPallocNoCache(int size, void *veOps, void *pVeopsSelf)
{
    return NULL;
}
...
static unsigned int SecureAllocGetVeAddrOffset()
{
    if(gAdapterContext != NULL)
        return gAdapterContext->phyOffset;
    else
    {
        loge("gAdapterContext is NULL, please call SecureAllocOpen\n");
        return 0;
    }
}

static int SecureAllocGetDebugInfo(char*  pDst, int nDstBufsize)
{
    logw("unsupported!!!!");
    return 0;
}

static int SecureAllocGetFd(void* pbuf)
{
    int flag = 0;
    ...
    aw_mem_list_for_each_entry(tmp, &gAdapterContext->list, i_list)
    {
        if (addr_vir >= tmp->tee_addr
            ...
            buffer_fd = tmp->buf_fd;
            logv("ion_alloc_get_fd, fd = 0x%08x tee_addr= 0x%08x\n", buffer_fd, addr_vir);
            flag = 1;
            break;
        }
    }
...
struct ScMemOpsS _SecureMemOpsS =
{
    ...
    .get_ve_addr_offset = SecureAllocGetVeAddrOffset,
    .get_debug_info    = SecureAllocGetDebugInfo,
    .get_vir_by_fd     = SecureAllocGetVirByFd,
    .get_phy_by_fd     = SecureAllocGetPhyByFd,
    .free_phy_by_fd    = SecureAllocFreePhyByFd,
    .get_fd_by_vir     = SecureAllocGetFd
};
